...
export default async function createApp(config: IConfig): Promise<Koa> {
  ...
  if (config.extensionDevelopmentPath) {
    ...
    app.use(
      kmount(
        ...
        kstatic(config.extensionDevelopmentPath, serveOptions)
      )
    );
  }
  ...
  if (config.build.type === "static") {
    app.use(
      kmount("/static/build", kstatic(config.build.location, serveOptions))
    );
  } else if (config.build.type === "sources") {
    console.log("Serving VS Code sources from " + config.build.location);
    app.use(
      kmount("/static/sources", kstatic(config.build.location, serveOptions))
    );
    app.use(
      kmount(
        "/static/sources",
        kstatic(
          join(config.build.location, "resources", "server"),
          serveOptions
        )
      )
    ); // for manifest.json, favicon and code icons.
    app.use(
      kmount(
        "/static/sources",
        kstatic(join(config.build.location, "remote"), serveOptions)
      )
    ); // for manifest.json, favicon and code icons.
    ...
    config.extensionPaths.forEach((extensionPath, index) => {
      ...
      app.use(
        kmount(
          ...
          kstatic(extensionPath, serveOptions)
        )
      );
    });
  }
