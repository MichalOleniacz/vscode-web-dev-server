...
class Workbench {
  constructor(
    ...
    private readonly builtInExtensions: IScannedBuiltinExtension[] = []
  ) {}
  ...
  async render(workbenchWebConfiguration: IWorkbenchOptions): Promise<string> {
    ...
    try {
      const workbenchTemplate = (
        await fs.readFile(path.resolve(__dirname, "../../../index.html"))
      ).toString();
      return workbenchTemplate.replace(
        /\{\{([^}]+)\}\}/g,
  ...
  async renderCallback(): Promise<string> {
    return await fetch(
      `${this.baseUrl}/out/vs/code/browser/workbench/callback.html`
    );
  }
}
...
export default function (config: IConfig): Router.Middleware {
  ...
  router.use(async (ctx, next) => {
    if (config.build.type === "sources") {
      const builtInExtensions = await getScannedBuiltinExtensions(
      ...
      ctx.state.workbench = new Workbench(
        `${ctx.protocol}://${ctx.host}/static/build`,
        false
      );
